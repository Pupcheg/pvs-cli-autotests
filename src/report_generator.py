...
def generate_report(
    ...
    def get_status_text(status: str) -> str:
        """Возвращает текстовое обозначение для статуса."""
    ...
    for i, r in enumerate(results, start=1):
        ...
        lines.append(f"- **Вывод:** {get_status_text(status)}")
        lines.append("")
    ...
    for i, r in enumerate(results, start=1):
        ...
        status = r.get('status', 'неизвестно')
        lines.append(f"| {i} | {case_name} | {direction} | {get_status_text(status)} |")
    ...
    lines.append(f"- Соответствует: **{passed}**")
    if partial > 0:
        lines.append(f"- Частично соответствует: **{partial}**")
    if failed > 0:
        lines.append(f"- Не соответствует: **{failed}**")
